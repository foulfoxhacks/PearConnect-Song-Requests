  ...
  <main class="session-page">
    ...
    <div class="session-layout">
      <section class="session-workflow" aria-label="Request a song">
        ...
        <section v-if="receipt" class="session-receipt" aria-live="polite"><p class="session-eyebrow">YOUR REQUEST RESULT</p><h2>{{ receipt.code === 'added' ? 'Enqueue confirmed.' : receipt.state === 'done' ? 'Your request was checked.' : receipt.state === 'uncertain' ? 'Check before retrying.' : receipt.state === 'checking' ? 'Checking your song…' : receipt.state === 'received' ? 'Request received.' : receipt.state === 'sending' ? 'Sending…' : 'Request not added.' }}</h2><p>{{ receipt.message }}</p><p v-if="receipt.code === 'added'">The player will play it when it reaches your song in the queue.</p><button type="button" @click="checkReceipt">Check request status</button><template v-if="!['sending','received','checking'].includes(receipt.state)"><label v-if="receipt.state === 'uncertain' || receipt.outcomeUncertain" class="session-checkbox"><input v-model="acknowledged" type="checkbox">I checked with the streamer before sending another request.</label><button class="session-text-button" :disabled="(receipt.state === 'uncertain' || receipt.outcomeUncertain) && !acknowledged" @click="another">Request another song</button></template></section>
      </section>
